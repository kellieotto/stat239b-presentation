...
\frame{
\bit
\item In the final version of the paper, Abadie and Imbens suggest that this is the first example for which the bootstrap is inconsistent for a statistic that is asymptotically normal, $\sqrt{n}$-consistent and asymptotically unbiased.

\item As far as we know, this is true, but it may not be of substantial interest because unbiasedness seems not to have any relationship to the consistency of the bootstrap and it is easy to construct much simpler examples satisfying stronger bias conditions.
