...
\bit
\item The bootstrap will never work for this kind of matching estimator with a single match on one covariate
\item We can save the bootstrap by making the estimator ``more smooth'':
\bit
\item Multiple matches
\item Subsampling
\item $m$ out of $n$ bootstrap
\eit
\item Just because the bootstrap doesn't work does not mean that their asymptotic result will work for finite samples!
\eit
}
